...
class ModeDecider:
    ...
    def update(self, face_detected: bool) -> str:
        ...
        if face_detected:
            ...
            # Face lost - check if it's been gone long enough to switch
            if self._confirmed_tracking and self._face_last_seen is not None:
                time_since_face = current_time - self._face_last_seen
                if time_since_face >= self.threshold_seconds:
                    self._face_first_seen = None
                    self._confirmed_tracking = False
                    return "idle"
